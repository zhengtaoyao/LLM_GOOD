...
def train(args):
    ...
    def train2(model, optimizer, iter):
        ...
        for epoch in range(args.epochs):
            ...
            # train the OOD filter with sudo-labels from LLM
            sup_loss = xent(logits[data.detection_mask_train2], data.joint_y_all2[data.detection_mask_train2])
            loss += sup_loss
            loss.backward()
            optimizer.step()
            # validate
        ...
        for epoch in range(args.epochs):
            ...
            if epoch % 10 == 0:
                ...
                with torch.no_grad():
                    embeds, logits = model(data)
                preds = logits.argmax(axis=1).detach()
                val_acc = get_acc(data.y_origin, preds, data.val_mask)
                ent_loss = ent_loss_func(logits).detach().cpu()
    ...
    def evaluate(model):
        ...
        with torch.no_grad():
            ...
            embeds, logits = model(data)
            preds = logits.argmax(axis=1).detach()
            test_acc = get_acc(data.y_origin, preds, data.test_mask)
    ...
    budget_ad_llm = len(args.ID_classes) * (n_samples_per_class-n_samples_init) * 5
    
    idx_cand_an = left_idx_all
    cluster_num = 48

    # re-map labels
    label_map = {y : i for i, y in enumerate(set(ID_classes))}
    
    # ID classification y
    ID_y_llm = sudo_labels.clone()
    all_indices = torch.arange(0, len(data.y)).cuda()
    
    idx_cand_llm = [idx for idx,itm in enumerate(sudo_labels) if itm != -1]
    #idx_cand_an is initial pool for human to choose, idx_cand_an2 is for llm to choose to train the ood filter
    idx_cand_an2 = list(set(idx_cand_an).intersection(set(idx_cand_llm)))

    # idx_cand_an = list(set(idx_cand_an).intersection(set(idx_cand_llm)))
    joint_y_llm = [label_map.get(y.item(), len(ID_classes)) for y in sudo_labels]
    random_indices_llm = torch.randperm(len(idx_cand_an2))[:budget_ad_llm]
    random_elements = torch.tensor(idx_cand_an2)[random_indices_llm]
    idx_selected_llm = random_elements.cuda()
    data.detection_mask_train2 = torch.isin(all_indices, torch.tensor(idx_selected_llm))
    data.joint_y_all2 = joint_y_llm #joint means id and ood together, so far all ids are llm's
    data.joint_y_all2 = torch.tensor(data.joint_y_all2).cuda()
    
    # In extreme case, combine noisy labels with small amount of human annotated accurate labels 
    ID_y_noisy = sudo_labels.clone()
    for i in range(data.y.shape[0]):
        ID_y_noisy[i] = label_map.get(sudo_labels[i].item(), 0)
    data.y_noisy = ID_y_noisy
    ...
    for iter in range(iter_num + 1):
        ...
        # Use accurate labels to train ID classifier
